...
public class ImageHandleHelper {
    ...
    public static void drawStringForImage(String filePath, String content,
                                          Color contentColor, float qualNum,String targetFile, int x, int y) {
        ...
        // 写入文字
        g.drawString(content, width/2+x, height/2+y);
        g.dispose();
        ...
        try {
            ...
            ImageIO.write(bimage, /*"GIF"*/ formatName /* format desired */ , new File(targetFile) /* target */ );
            //out.flush();
        } catch (Exception e) {
     ...
     */
    public static void cutImage(String srcFile, String targetFile, int startAcross,
                                int StartEndlong, int width, int hight) throws Exception {
        // 取得图片读入器
